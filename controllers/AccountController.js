...
const postLogin = async (req, res) => {
  ...
  try {
    const data = await Usuarios.autenticarUsuario(
      login.nomeUsuario,
      login.senha
    );
...
const getUsuarios = async (req, res) => {
  try {
    ...
    if (data.statusCode === 200 && data.isSuccess) {
      ...
      throw new Error(
        data.errorMessages?.join("<br>") || "Erro ao Buscar Usuários"
      );
    }
  } catch (error) {
    req.session.error = {
      title: "Problema ao Carregar Usuários",
      message: error,
    };
...
const deleteUsuario = async (req, res) => {
  ...
  try {
    ...
    if (data.isSuccess)
      ...
      throw new Error(
        data.errorMessages?.join("<br>") || "Erro ao tentar Excluir Usuário"
      );
  } catch (error) {
    console.error("Erro ao excluir usuário:", error);
    req.session.error = {
      title: "Erro ao Tentar Excluir a Usuário",
      message: error,
    };
  }
  return res.redirect("/usuarios");
  ...
  try {
    ...
    if (data.isSuccess) {
      ...
      return res.render("account/reset", {
        ...
        data: reset,
        error: {
          title: "Problemas ao resetar Senha",
          message: data.errorMessages?.join("<br>"),
        },
      });
    }
