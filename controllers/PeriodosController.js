...
const getPeriodosSafra = async (req, res) => {
  try {
    ...
    if (data.statusCode === 200 && data.isSuccess) {
      ...
      throw new Error(
        data.errorMessages?.join("<br>") || "Erro ao Buscar Períodos Safra"
      );
    }
  } catch (error) {
    req.session.error = {
      title: "Problema ao Carregar Períodos",
      message: error,
    };
...
const postGerarPeriodos = async (req, res) => {
  ...
  try {
    ...
    if (data.isSuccess) {
      req.session.success = {
        title: `Safra ${dados.anoSafra}`,
        message: `Períodos Gerados com Sucesso!`,
      };
    } else {
      req.session.error = {
        title: `Safra ${dados.anoSafra}`,
        message: data.errorMessages?.join("<br>") || "Erro ao Gerar Períodos.",
      };
    }
...
const deletePeriodo = async (req, res) => {
  const { id } = req.params;
  try {
    const retorno = await Periodos.getPeriodo(req, id);
    console.log(retorno);
    const safraId = retorno.result.safraId;
    const data = await Periodos.deletePeriodo(req, id);
    if (data.isSuccess)
      req.session.success = {
        title: "Período excluído com Sucesso!",
        message: "A exclusão foi realizada com êxito.",
      };
    else
      throw new Error(
        data.errorMessages?.join("<br>") || "Erro ao tentar Excluir Período"
      );
    return res.redirect(`/safras/periodos/${safraId}`);
  } catch (error) {
    console.error("Erro ao excluir safra:", error);
    req.session.error = {
      title: "Erro ao Tentar Excluir a Período",
      message: error,
    };
  }
